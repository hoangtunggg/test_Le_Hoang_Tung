...
export function useTodos(filters: TodoFilters = {}) { const normalized = normalizedTodoFilters(filters); return useQuery({ queryKey: todoQueryKey(normalized), queryFn: async (): Promise<TodoListResponse> => { const params = Object.fromEntries(Object.entries(normalized).filter(([, value]) => value !== "")); return (await api.get("/todos", { params })).data; } }); }
